...
void ajouter_horaire(char *depart, char *arrivee, char *heure_depart, char *heure_arrivee, int distance)
{
    ...
    nouveau_noeud->horaire.depart = strdup(depart);
    nouveau_noeud->horaire.arrivee = strdup(arrivee);
    nouveau_noeud->horaire.heure_depart = strdup(heure_depart);
    nouveau_noeud->horaire.heure_arrivee = strdup(heure_arrivee);
...
void liberer_memoire()
{
    ...
    while (courant != NULL)
    {
        Noeud *temp = courant;
        courant = courant->suivant;
        free(temp->horaire.depart);
        free(temp->horaire.arrivee);
        free(temp->horaire.heure_depart);
        free(temp->horaire.heure_arrivee);
        free(temp);
    }
    liste_horaires = NULL;
}
